...
class TryOptim(Thread):
    ...
    def run(self):
        try:
            ...
            ymid_f = ymid_r + np.sin(self.init_data[2]) * paras["Car_L"]
            ymid_f = ymid_f + np.sin(self.init_data[2]) * (paras["Car_Length"] - paras["Car_L"]) / 2

            yf_r = ymid_f + np.sin(self.init_data[2] - np.pi / 2) * paras["Car_Width"] / 2
            yf_l = ymid_f + np.sin(self.init_data[2] + np.pi / 2) * paras["Car_Width"] / 2
            if yf_r < paras["Parking_Y"] or yf_l > (paras["Freespace_Y"] + paras["Parking_Y"]):
