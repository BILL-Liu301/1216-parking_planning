...
class Parking_Trajectory_Planner(nn.Module):
    ...
    def cal_state(self, network, main_norm, direction):
        ...
        # # 测试状态转换算法正确性
        # state[:, :, 0:1] = torch.ones(state[:, :, 0:1].shape) * 0.1
        # state[:, :, 1:2] = torch.ones(state[:, :, 1:2].shape) * 0.1 * direction
    ...
    def forward(self, inp_start_point):
        ...
        for step, model_per_step in enumerate(self.planners):
            ...
            for _ in range(self.num_anchor_per_step - 1):
                ...
                state_now = self.cal_state(model_per_step['decode_mean'], main_norm, direction)
                anchors_now = self.pre_from_state(state_now, anchor_last, direction)
                var_now = model_per_step['decode_var'](main_norm)
                # cumsum = torch.cumsum(main_lstm, dim=1)
                # anchors_now = model_per_step['decode_mean'](cumsum) + anchor_last
                # var_now = model_per_step['decode_var'](cumsum)
...
class Parking_Trajectory_Planner_LightningModule(pl.LightningModule):
    def __init__(self, paras: dict):
        ...
        self.criterion_val = Criterion_Dis(car_length=paras['car_length'], weight=0.8, reduction='max')
        self.criterion_test_dis = Criterion_Dis(car_length=paras['car_length'], weight=0.8, reduction='none')
