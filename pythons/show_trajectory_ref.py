...
from api.base.paths import path_solutions, path_figs_all
...
if __name__ == '__main__':
    ...
    for solution in tqdm(solutions, desc='Trajectory', leave=False, ncols=100, disable=False):
        sol = np.loadtxt(os.path.join(path_solutions, solution))

        xmid_r = sol[:, 2]
        ymid_r = sol[:, 3]
        xmid_f = xmid_r + np.cos(sol[:, 4]) * paras_base["Car_L"]
        ymid_f = ymid_r + np.sin(sol[:, 4]) * paras_base["Car_L"]
        xmid_r = xmid_r - np.cos(sol[:, 4]) * (paras_base["Car_Length"] - paras_base["Car_L"]) / 2
        ymid_r = ymid_r - np.sin(sol[:, 4]) * (paras_base["Car_Length"] - paras_base["Car_L"]) / 2
        xmid_f = xmid_f + np.cos(sol[:, 4]) * (paras_base["Car_Length"] - paras_base["Car_L"]) / 2
        ymid_f = ymid_f + np.sin(sol[:, 4]) * (paras_base["Car_Length"] - paras_base["Car_L"]) / 2

        xr_r = xmid_r + np.cos(sol[:, 4] - np.pi / 2) * paras_base["Car_Width"] / 2
        yr_r = ymid_r + np.sin(sol[:, 4] - np.pi / 2) * paras_base["Car_Width"] / 2
        xr_l = xmid_r + np.cos(sol[:, 4] + np.pi / 2) * paras_base["Car_Width"] / 2
        yr_l = ymid_r + np.sin(sol[:, 4] + np.pi / 2) * paras_base["Car_Width"] / 2
        xf_r = xmid_f + np.cos(sol[:, 4] - np.pi / 2) * paras_base["Car_Width"] / 2
        yf_r = ymid_f + np.sin(sol[:, 4] - np.pi / 2) * paras_base["Car_Width"] / 2
        xf_l = xmid_f + np.cos(sol[:, 4] + np.pi / 2) * paras_base["Car_Width"] / 2
        yf_l = ymid_f + np.sin(sol[:, 4] + np.pi / 2) * paras_base["Car_Width"] / 2
